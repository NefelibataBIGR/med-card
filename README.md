...
导入教材 PDF，按段落提取主概念，并生成一张张的复习卡片。卡片包含：
...
MED_CARD_LLM_API_KEY=你的_deepseek_api_key
```

`.env.example` 里还提供了这些可选项：

- `MED_CARD_LLM_BASE_URL`：Base_url，DeepSeek的默认已给出
- `MED_CARD_LLM_MODEL`：模型选择，默认v4-flash
